 ...
 * use by dttsp.
 *
 * This program is based on other programs:
 * (1) rtl_fm.c
 ...
 *
 * Full source for rtl-sdr can be obtained from:
 ...
 *
 * This program is based on rtl_fm and feeds 16-bit integer audio samples to
 * the JACK audio connection kit for use by dttsp.
 *
 * Suggestion for use:
 * ./rtl_j -p 50 -F 9 -f 144.39e6 -M raw -A lut -l 0 -s 48000 -u 19004 - 
 *
 * then:
 * jack_connect rtl:q sdr:il;jack_connect rtl:i sdr:ir
 *
 * Bugs:
 *  I'm not sure that tuning is quite right.  It seems to tune to the lower
 *  side of the center frequency
 *  Most on-air testing has been done watching data packets, not sideband.
 *
 */
...
void usage(void)
{
	fprintf(stderr,
		...
		"\t[-u port# UDP port for commands (default: 8192)]\n"
...
static void *output_thread_fn(void *arg)
{
	...
	while (!do_exit) {
		...
		// write to jack
		want = s->result_len * sizeof(int16_t);
		available = jack_ringbuffer_write_space(iqdata_out);
		if (available >= want) {	/* All or nothing */
			jack_ringbuffer_write(iqdata_out, (const char*)s->result, want);
		} else {
			printf("output_thread: want %d available %d\n", want, available);
		}
...
{
	struct jackparams *p;
	uint want;			/* count of samples wanted */
	uint get;			/* samples to get */
	uint available;			/* bytes available */
	uint have;			/* samples available */
	...
	have = available / sizeof(int16_t);
	if (have >= want) {
		get = want;
	} else {
		get = 0;
		printf("j: want %d have %d\n", want, have);
	}
	jack_ringbuffer_read(iqdata_out, p->buffer, get * sizeof(int16_t));
	input = (int16_t *)p->buffer;
...
start_jack(char *portname, uint32_t samp_rate)
{
	...
	iqdata_out = jack_ringbuffer_create (RB_SIZE);
...
static void optimal_settings(int freq, int rate)
{
	...
	//if(dm->downsample > 8) dm->downsample = 8;
...
static void *controller_thread_fn(void *arg)
{
	...
	fprintf(stderr, "Downsample passes: %i.\n", demod.downsample_passes);
...
int main(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, "d:f:g:s:b:l:o:t:r:p:E:F:A:M:u:h")) != -1) {
		switch (opt) {
		case 'd':
...
interpret(char *str)
{
	...
	if (!strcmp(*sp, "set")) {
		...
		if (!strcmp(*sp, "freq")) {
			...
			printf("set freq %s\n", *sp);
			free(saved_freq);
			saved_freq = strdup(*sp);

			newfreq = atofs(*sp);
			newfreq *= 1000000;
			dongle.freq = (int) newfreq;
			optimal_settings((int)newfreq, demod.rate_in);
			printf("Tune: %u -> %u\n", (int)newfreq, dongle.freq);
			verbose_set_frequency(dongle.dev, dongle.freq);
			//verbose_set_frequency(dongle.dev, (int)newfreq);
			return("ok");
		} else if (!strcmp(*sp, "ppm")) {
			sp++;
			dongle.ppm_error = atoi(*sp);
			verbose_ppm_set(dongle.dev, dongle.ppm_error);
			return("ok");
		} else if (!strcmp(*sp, "autogain")) {
			verbose_auto_gain(dongle.dev);
			return("ok");
		} else if (!strcmp(*sp, "gain")) {
			sp++;
			dongle.gain = (int)(atof(*sp) * 10);
			dongle.gain = nearest_gain(dongle.dev, dongle.gain);
			verbose_gain_set(dongle.dev, dongle.gain);
			return("ok");
